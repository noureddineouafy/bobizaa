            ...
            if (typeof plugin.all === 'function') {
                try {
                    ...
                    for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                        ...
                        if (data.exists)
                            await m.reply(`*[ ⚠️ 𝚁𝙴𝙿𝙾𝚁𝚃𝙴 𝙳𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝙲𝙾𝙽 𝙵𝙰𝙻𝙻𝙾𝚂 ⚠️ ]*\n\n*—◉ 𝙿𝙻𝚄𝙶𝙸𝙽:* ${name}\n*—◉ user:* ${m.sender}\n*—◉ cmd:* ${m.text}\n\n*—◉ error:*\n\`\`\`${format(e)}\`\`\`\n\n*[❗] reported to noureddine_ouafy #reporte*`.trim(), data.jid)
                    }
                }
            }
            if (!opts['restrict'])
...
export async function callUpdate(callUpdate) {
    ...
    if (nk.isGroup == false) {
    if (nk.status == "offer") {
    let callmsg = await this.reply(nk.from, `مرحبا عزيزي/عزيزتي *@${nk.from.split('@')[0]}*, 😣 ${nk.isVideo ? 'videollamadas' : 'المكالمات'}\nللاسف الشديد سيتم حظرك (عمل لك بلوك) لأنك لم تحترم معايير استخدام البوت وقد قمت بالاتصال دون علم صاحبه نورالدين ونورالدين يكره من يتصل ببوتاته \n اذا اردت فك الحظر تواصل مع نورالدين \ninstagram.com/noureddine_ouafy`, false, { mentions: [nk.from] })
    //let data = global.owner.filter(([id, isCreator]) => id && isCreator)
    //await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
    let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;BOBIZA BOT 👑;;;\nFN:BOBIZA BOT 👑\nORG:BOBIZABOT 👑\nTITLE:\nitem1.TEL;waid=212605784394:+212605784394\nitem1.X-ABLabel:𝐁𝐫𝐮𝐧𝐨 𝐒𝐨𝐛𝐫𝐢𝐧𝐨 👑\nX-WA-BIZ-DESCRIPTION:instagram.com/noureddine_ouafy.\nX-WA-BIZ-NAME:BOBIZABOT 👑\nEND:VCARD`
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        premium: '*هذا الأمر للذين يشاركون منشوراتي في الأنستغرام اذهب الآن وشارك المنشورات هناك بكثرة . ثم راسل نورالدين في حسابه  مرفوقا بسكرينات أنك نشرت منشوراته على الإنستغرام*\n\ninstagram.com/noureddine_ouafy',
        group: 'عذرا ولكن هذا الأمر خاص بالمجموعات \n وايضا خاص بصاحب البوت فقط \n لن يشتغل معك الامر لا تحاول معه \n تابع صاحب البوت من هنا :\n instagram.com/noureddine_ouafy',
        private: '*هذا الامر يشتغل في الخاص فقط*',
        admin: '*معذرة يا صديقي لكن هذا الأمر بأدمين هذه المجموعة وخاص بنورالدين فقط لذا لا تحاول مجددا تجربته لانه لن يشتغل معك *',
        botAdmin: '*يجب على البوت ان يكون ادمين في هذه المجموعة حتى يشتغل هذا الامر*',
