...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	...
	if (!text) throw 'تحميل الصور من موقع \nhttps://www.pinterest.com/\n\nنكتب هكذا على سبيل المثال :\n*.pint* naruto'
	let res = await pinterest(text)
	// if (!res) throw res
	let mime = await lookup(res)
	text.match(URL_REGEX) ?
		await conn.sendMessage(m.chat, { [mime.split('/')[0]]: { url: res }, caption: `تم التحميل بنجاح: ${await shortUrl(res)}` }, { quoted: m }) : await conn.sendFile(m.chat, res, '', `النتيجة  : ${text.capitalize()}\n${await shortUrl(res)}`, m)
}
