    ...
    let _text = [
      ...
      ...Object.keys(tags).map(tag => {
        return header.replace(/%category/g, tags[tag]) + '\n' + [
          ...help.filter(owner => owner.tags && owner.tags.includes(tag) && owner.help).map(onwer => {
            return menu.help.map(help => {
              return body.replace(/%cmd/g, owner.prefix ? help : '%p' + help)
                .replace(/%isdiamond/g, owner.diamond ? '(Ⓛ)' : '')
                .replace(/%isPremium/g, owner.premium ? '(Ⓟ)' : '')
                .trim()
            }).join('\n')
          }),
          footer
...
renderLargerThumbnail: true
    ...
    conn.reply(m.chat, ' هناك خطأ في لائحة', m)
    throw e
  }
}
handler.help = ['owner']
