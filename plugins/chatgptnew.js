...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  ...
  if (!text) throw `هذا الامر خاص بالذكاء الاصطناعي يمكنك من تسأل البوت اي سؤال تريده مثلا :\n\n*.chatgptnew* ما هي أول آية أنزلت على الرسول محمد صلى الله عليه وسلم`;
  try {
        conn.sendPresenceUpdate('composing', m.chat);
        //let sistema1 = await fetch(`https://raw.githubusercontent.com/Skidy89/chat-gpt-jailbreak/main/Text.txt`).then(v => v.text());
        let sistema1 = `instagram.com/noureddine_ouafy`;
      ...
      try {
        conn.sendPresenceUpdate('composing', m.chat);
        const syms1 = `instagram.com/noureddine_ouafy`;
